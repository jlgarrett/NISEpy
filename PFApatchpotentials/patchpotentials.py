...
def patches_on_sphere3(sphere, filtered_patch_dict):
    heights = list(filtered_patch_dict.keys())
    heights.sort()
    va = np.arange(len(heights))
    patches_out = np.zeros(filtered_patch_dict[heights[0]].shape)

    smap = np.interp(sphere, heights, va)
    for i in range(len(heights)):
        hloc = heights[i]
        amap = smap - i
        amap[amap>1]=2-(amap[amap>1])
        amap[amap<0]=0
        patches_out[:,:] += filtered_patch_dict[hloc]*amap
